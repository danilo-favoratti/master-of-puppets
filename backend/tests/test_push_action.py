"""
Test script to load a tiny 3x3 game, push a container, and verify the result.
This version tests all possible push scenarios including edge cases.
"""
import os
import sys
...
def test_push_successful(engine):
    """Test successful push scenarios (all 4 directions)"""
    tests = [
        # [player_x, player_y, container_x, container_y, direction]
        # Push down
        [1, 0, 1, 1, (0, 1)],
        # Push up
        [1, 2, 1, 1, (0, -1)],
    ...
    directions_names = ["down", "up", "right", "left"]
    
    for i, test in enumerate(tests):
        player_x, player_y, container_x, container_y, direction = test
        
        print(f"\n=== Test {i+1}: Push {directions_names[i]} ===")
        ...
        # Perform the push using the Person.push method
        if game.player:
            result = game.player.push(game.board, container_position, direction)
...
def test_push_failures(engine):
    """Test push failure scenarios"""
    tests = [
        # [desc, player_x, player_y, container_x, container_y, direction]
        # Object would go off the board
        ["Object off board (top)", 1, 1, 1, 0, (0, -1)],
        ["Object off board (bottom)", 1, 1, 1, 2, (0, 1)],
        ["Object off board (left)", 1, 1, 0, 1, (-1, 0)],
        ["Object off board (right)", 1, 1, 2, 1, (1, 0)],
        
        # Object and player not adjacent
        ["Not adjacent (diagonal)", 0, 0, 1, 1, (1, 1)],
        ["Not adjacent (too far)", 0, 0, 2, 0, (1, 0)],
    ]
    
    for i, test in enumerate(tests):
        desc, player_x, player_y, container_x, container_y, direction = test
        ...
        # Attempt to push using the Person.push method
        if game.player:
            result = game.player.push(game.board, container_position, direction)
            
            print(f"Push attempt result: {'Failed as expected' if not result['success'] else 'Unexpectedly succeeded'}")
            print(f"Message: {result['message']}")
            
            # Show the board state
            print("Board state after push attempt:")
            for y in range(game.board.height):
                row = ""
                for x in range(game.board.width):
                    entities = game.board.get_entities_at((x, y))
...
def test_blocked_push(engine):
    """Test push where the target position is blocked by another object"""
    print("\n=== Test: Blocked Push ===")
    ...
    # Create a blocking object at (2,1)
    blocking_obj = {"id": "blocking_obj", "type": "furniture", "name": "Blocking Object", 
                    "position": {"x": 2, "y": 1}, "is_movable": False}
    engine.entities["blocking_obj"] = blocking_obj
    engine.board[1][2] = "furniture"
    engine.entity_positions[(2, 1)] = "blocking_obj"
    
    print("Added blocking object at (2,1)")
    engine.print_board()
    ...
    # Try to push container using the Person.push method
    if game.player:
        # Push right
        result = game.player.push(game.board, container_position, (1, 0))
    ...
    if "blocking_obj" in engine.entities:
        ...
        engine.board[1][2] = None
        del engine.entity_positions[(2, 1)]

def test_load_custom_examples():
    """Test loading custom example files for push testing"""
    example_files = [
        ...
        if os.path.exists(file_path):
            ...
            if engine:
                ...
                if engine.width >= 3 and engine.height >= 3:
                    ...
                    print("Board too small for push tests (need at least 3x3)")
...
def main():
    """Run all push action tests"""
    print("=== PUSH ACTION TEST SUITE ===")
