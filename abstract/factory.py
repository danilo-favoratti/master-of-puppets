...
class LandObstacleFactory:
    """Factory class for creating various land obstacles."""
    
    @staticmethod
    def create_hole(name: str = "Hole", **props) -> GameObject:
        """Create a hole obstacle that players can jump over."""
        hole_props = {
            "name": name,
            "is_jumpable": True,
            "is_movable": False,
            "is_collectable": False,
            "weight": 0  # Holes don't have weight
        }
        
        # Create a copy of props to avoid modifying the original
        final_props = props.copy()
        
        # Update final_props with hole_props, but don't override existing props
        for key, value in hole_props.items():
            if key not in final_props:
                final_props[key] = value
        
        return GameObject(
            id=f"hole_{random.randint(1000, 9999)}",
            **final_props
        )
    
    @staticmethod
    def create_fallen_log(size: str = "medium", **props) -> GameObject:
        """Create a fallen log obstacle.
        
        Args:
            size: Size of the log ("small", "medium", or "large")
        """
        weights = {"small": 5, "medium": 10, "large": 20}
        names = {"small": "Small Fallen Log", "medium": "Fallen Log", "large": "Massive Fallen Log"}
        
        log_props = {
            "name": names.get(size, "Fallen Log"),
            "is_jumpable": True,
            "is_movable": size != "large",  # Only large logs can't be moved
            "is_collectable": False,
            "weight": weights.get(size, 10)
        }
        
        # Create a copy of props to avoid modifying the original
        final_props = props.copy()
        
        # Update final_props with log_props, but don't override existing props
    ...
    def create_tree_stump(height: int = 1, **props) -> GameObject:
        """Create a tree stump obstacle.
        
        Args:
            height: Relative height of the stump (1-3)
        """
        if not 1 <= height <= 3:
            height = 1
            
        return GameObject(
            id=f"stump_{random.randint(1000, 9999)}",
            name=f"Tree Stump (Height: {height})",
            is_jumpable=True,
            is_movable=False,
            is_collectable=False,
            weight=height * 5,
...
class LogStoolFactory:
    ...
    def create_stool(cls, variant: str = "squat", id: str = None) -> Dict[str, Any]:
        """
        Create a log stool object.
        
        Args:
            variant (str): The variant of stool to create (currently only "squat").
            id (str, optional): An optional id for the stool. If not provided, a default is generated.
            
        Returns:
            Dict[str, Any]: A dictionary representation of the log stool.
